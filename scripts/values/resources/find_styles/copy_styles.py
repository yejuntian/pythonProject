...
def startCopyString(from_dir, to_dir):
    getNameList("styles.json")
    getNameList("other_styles.json")
    publicFilePath = os.path.join(to_dir, "res/values/public.xml")
    getInsertNameList(publicFilePath, "style")
    travelFolderCopyAttr(from_dir, to_dir, "styles.xml")
    insertPublic(publicFilePath, "style")
