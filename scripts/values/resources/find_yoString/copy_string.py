...
def startCopyString(from_dir, to_dir):
    getNameList("string_list.json")
    getNameList("other_string.json")
    publicFilePath = os.path.join(to_dir, "res/values/public.xml")
    getInsertNameList(publicFilePath, "string")
    travelFolderCopyAttr(from_dir, to_dir, "strings.xml")
    insertPublic(publicFilePath, "string")
